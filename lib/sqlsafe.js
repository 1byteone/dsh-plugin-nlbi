...
function fallbackCheckTableAllowlist(sql, list) {
  // 降级方案：使用 shared.js 的正则提取表名（ESM 安全）
  // 这是降级路径，只有当 node-sql-parser AST 解析失败时才会触发
  return { ok: false, denied: ['(SQL 解析失败，无法校验表名)'], tables: [] }
}
...
export function validateAndPrepare(sql, options = {}) {
  ...
  try {
    ...
    // AST 解析失败：降级到 shared.js 正则检测（ESM 安全导入）
    // 说明：AST 解析失败说明 SQL 不可靠，直接拒绝比放行更安全
    return { ok: false, error: 'SQL 语法无法解析，已拒绝执行' }
  }
