 ...
 * 基于 dsh-mysql (v0.1.4) 二次开发：连接管理 + Text2SQL + BI 图表 + 报表 + Dashboard + 指标管理。
 *
 * 职责：
 *  1. 连接管理：$DSH_HOME/storages/dsh-plugin-nlbi/connections.json 持久化多套
 *     MySQL 连接（名称/host/port/user/password/默认库/表白名单/写权限/行级权限/列级黑名单），
 ...
 *     Text2SQL 使用指引 + 指标/维度上下文，所有预设的模型无需任何预设改动即可感知当前数据库；
...
function auditLog(entry) {
  ...
  // 环形缓冲：超出上限删除最旧的
  while (auditEntries.length > MAX_AUDIT_ENTRIES) {
    auditEntries.shift()
  }
}
 ...
 * @returns {string} WHERE 子句片段（不含 WHERE 关键字），空字符串表示无限制
 */
function buildRowFilterClause(connection) {
  const perms = connection && connection.permissions
  if (!perms || !Array.isArray(perms.rowFilters) || perms.rowFilters.length === 0) return ''

  const conditions = []
  for (const filter of perms.rowFilters) {
    if (!filter.column || !filter.operator) continue
    const col = '`' + filter.column.replace(/`/g, '') + '`'
    switch (filter.operator) {
      case '=':
        if (filter.value !== undefined && filter.value !== null && filter.value !== '') {
          conditions.push(col + " = '" + String(filter.value).replace(/'/g, "''") + "'")
        }
        break
      case '!=':
        if (filter.value !== undefined && filter.value !== null && filter.value !== '') {
      ...
      case 'IN':
        if (Array.isArray(filter.values) && filter.values.length > 0) {
          const inList = filter.values.map(v => "'" + String(v).replace(/'/g, "''") + "'").join(', ')
          conditions.push(col + ' IN (' + inList + ')')
        }
        break
      case 'LIKE':
        if (filter.value !== undefined && filter.value !== null && filter.value !== '') {
          conditions.push(col + " LIKE '" + String(filter.value).replace(/'/g, "''") + "'")
        }
        break
    }
  }
 ...
 * @returns {boolean} true = 列被禁止
 */
function isColumnBlacklisted(columnName, connection) {
  const perms = connection && connection.permissions
  if (!perms || !Array.isArray(perms.columnBlacklist) || perms.columnBlacklist.length === 0) return false
  return perms.columnBlacklist.includes(columnName.toLowerCase())
}
...
async function nlQuery(args) {
  ...
  try {
    ...
    if (!result.ok) {
      auditLog({ type: 'nl_query', connectionId: connection.id, question, success: false, error: result.error, duration })
    ...
    const chart = suggestChartForResult(result.result, question, args && typeof args.chartType === 'string' ? args.chartType : (result.chartType || undefined))
    auditLog({ type: 'nl_query', connectionId: connection.id, question, sql: result.sql, success: true, rowCount: result.result.rowCount, duration })
    ...
    auditLog({ type: 'nl_query', connectionId: connection.id, question, success: false, error: String(err.message || err), duration: Date.now() - started })
...
async function loadReports() {
  try {
    ...
    const parsed = JSON.parse(text)
    if (parsed && !Array.isArray(parsed) && Array.isArray(parsed.reports)) return parsed.reports
    const error = new Error('报表存储格式无效：顶层必须包含 reports 数组')
    error.code = 'INVALID_REPORT_STORAGE'
    throw error
  } catch (err) {
    if (err && (err.code === 'ENOENT' || err.code === 'FS_NOT_FOUND')) return []
    bootLog('reports load failed: ' + String((err && err.message) || err))
    throw err
  }
}
...
async function listReports() {
  try {
    ...
    return { ok: false, error: '报表列表加载失败：' + String((err && err.message) || err) }
  }
}
...
async function listMetricsService() { return metrics.listMetrics() }
async function saveMetricService(args) { return metrics.saveMetric(args && args.metric) }
async function deleteMetricService(args) { return metrics.deleteMetric(args && args.metricId) }

async function listDimensionsService() { return metrics.listDimensions() }
async function saveDimensionService(args) { return metrics.saveDimension(args && args.dimension) }
async function deleteDimensionService(args) { return metrics.deleteDimension(args && args.dimensionId) }

async function listDatasetsService() { return metrics.listDatasets() }
async function saveDatasetService(args) { return metrics.saveDataset(args && args.dataset) }
async function deleteDatasetService(args) { return metrics.deleteDataset(args && args.datasetId) }

async function getMetricSuggestionsService(args) {
  // 根据当前连接的表结构推荐指标
  const configs = await ensureLoaded()
  const connectionId = args && typeof args.connectionId === 'string' ? args.connectionId : ''
  const cfg = configs.connections.find(c => c.id === connectionId)
  if (!cfg) return { ok: false, error: '连接不存在' }
  const got = await getPool(configs, connectionId)
  if (got.error !== undefined) return { ok: false, error: got.error }
  ...
  try {
    ...
    // 智能推荐：找到可能的数值列作为指标候选
    const suggestions = []
    for (const table of schemaResult.schema) {
      for (const col of table.columns) {
        if (/\b(int|bigint|decimal|float|double|numeric|real)\b/i.test(col.type) && !/tinyint\(1\)/i.test(col.type)) {
          const snakeToCamel = (s) => s.replace(/_([a-z])/g, (_, c) => c.toUpperCase())
          suggestions.push({
            name: snakeToCamel(col.column),
            expression: 'SUM(`' + table.name + '`.`' + col.column + '`)',
            sourceTable: table.name,
            sourceColumn: col.column,
            type: 'sum',
            description: (table.comment || table.name) + '.' + (col.comment || col.column) + ' 求和',
          })
        }
      }
    }
...
async function listDashboardsService() { return dashboard.listDashboards() }
async function getDashboardService(args) { return dashboard.getDashboard(args && args.dashboardId) }
async function saveDashboardService(args) { return dashboard.saveDashboard(args && args.dashboard) }
async function deleteDashboardService(args) { return dashboard.deleteDashboard(args && args.dashboardId) }
async function duplicateDashboardService(args) { return dashboard.duplicateDashboard(args && args.dashboardId) }
async function addWidgetService(args) { return dashboard.addWidget(args && args.dashboardId, args && args.widget) }
async function updateWidgetService(args) { return dashboard.updateWidget(args && args.dashboardId, args && args.widgetId, args && args.patch) }
async function removeWidgetService(args) { return dashboard.removeWidget(args && args.dashboardId, args && args.widgetId) }
async function moveWidgetService(args) { return dashboard.moveWidget(args && args.dashboardId, args && args.widgetId, args && args.position) }
async function updateFiltersService(args) { return dashboard.updateFilters(args && args.dashboardId, args && args.filters) }

async function executeDashboardQueryService(args) {
  const dashboardId = args && typeof args.dashboardId === 'string' ? args.dashboardId : ''
  const filterValues = args && typeof args.filterValues === 'object' ? args.filterValues : {}
  const dashResult = await dashboard.getDashboard(dashboardId)
  if (!dashResult.ok) return dashResult

  const configs = await ensureLoaded()
  const cfg = configs.connections.find(c => c.id === dashResult.dashboard.connectionId)
  if (!cfg) return { ok: false, error: 'Dashboard 关联的连接不存在' }
  const got = await getPool(configs, cfg.id)
  if (got.error !== undefined) return { ok: false, error: got.error }

  const result = await dashboard.executeDashboardQueries(got.pool, dashResult.dashboard, filterValues, {
    validateQuery: (sql) => {
      const validation = validateAndPrepare(sql, { allowlist: cfg.tables, maxRows: MAX_ROWS })
      return validation.ok ? { ok: true, sql: validation.safeSql } : { ok: false, error: validation.error }
    },
    buildChartSpec: (data, title, chartType) => suggestChartForResult(data, title, chartType),
  })
  return sanitizeForBoundary(result)
}

async function getDrillDownService(args) {
  const baseQuery = args && typeof args.baseQuery === 'string' ? args.baseQuery : ''
  const dimension = args && typeof args.dimension === 'string' ? args.dimension : ''
  const value = args && args.value !== undefined ? args.value : ''
  if (!baseQuery || !dimension) return { ok: false, error: '缺少查询或维度' }

  const drillSql = dashboard.buildDrillDownQuery(baseQuery, dimension, value)

  // 执行下钻查询
  const configs = await ensureLoaded()
  const sessionId = args && typeof args.sessionId === 'string' ? args.sessionId : undefined
  const connection = resolveSelection(configs, sessionId)
  if (!connection) return { ok: false, error: '未选择数据库连接' }
  const got = await getPool(configs, connection.id)
  if (got.error !== undefined) return { ok: false, error: got.error }

  try {
    const validation = validateAndPrepare(drillSql, { allowlist: connection.tables, maxRows: MAX_ROWS })
    if (!validation.ok) return { ok: false, error: validation.error }
    const [rows, fields] = await got.pool.query(validation.safeSql)
    const columns = Array.isArray(fields) ? fields.map(f => f.name) : []
    const list = Array.isArray(rows) ? rows.slice(0, MAX_ROWS).map(jsonSafe) : []
    const chart = suggestChartForResult({ columns, rows: list }, dimension + ' 下钻')
    return sanitizeForBoundary({ ok: true, sql: validation.safeSql, result: { columns, rows: list, rowCount: list.length }, chart: chart.ok ? chart.spec : null })
  } catch (err) {
    return { ok: false, error: String(err.message || err) }
  }
}
...
async function exportDataService(args) {
  const format = args && typeof args.format === 'string' ? args.format : 'csv'
  const columns = Array.isArray(args && args.columns) ? args.columns : []
  const rows = Array.isArray(args && args.rows) ? args.rows : []
  const options = args && typeof args.options === 'object' ? args.options : {}

  if (columns.length === 0) return { ok: false, error: '无数据可导出' }

  const result = exportResult(format, columns, rows, options)
  return { ok: true, content: result.content, mimeType: result.mimeType, extension: result.extension }
}

// ── ★ 新增：审计日志服务 ──────────────────────────────────────────────────

async function getAuditLogService(args) {
  const limit = args && typeof args.limit === 'number' ? Math.min(args.limit, 200) : 50
  const offset = args && typeof args.offset === 'number' ? Math.max(0, args.offset) : 0
  const entries = auditEntries.slice().reverse().slice(offset, offset + limit)
  return { ok: true, entries, total: auditEntries.length }
}

// ── ★ 新增：权限配置 ──────────────────────────────────────────────────────

async function updatePermissionsService(args) {
  const connectionId = args && typeof args.connectionId === 'string' ? args.connectionId : ''
  const permissions = args && typeof args.permissions === 'object' ? args.permissions : null
  if (!connectionId || !permissions) return { ok: false, error: '缺少参数' }

  const configs = await ensureLoaded()
  const idx = configs.connections.findIndex(c => c.id === connectionId)
  if (idx < 0) return { ok: false, error: '连接不存在' }

  configs.connections[idx].permissions = {
    rowFilters: Array.isArray(permissions.rowFilters) ? permissions.rowFilters : [],
    columnBlacklist: Array.isArray(permissions.columnBlacklist) ? permissions.columnBlacklist.filter(c => typeof c === 'string') : [],
    maxRows: Number.isFinite(Number(permissions.maxRows)) ? Number(permissions.maxRows) : MAX_ROWS,
    queryTimeout: Number.isFinite(Number(permissions.queryTimeout)) ? Number(permissions.queryTimeout) : 15000,
  }

  try {
    await persistConfigs(configs)
    return { ok: true, permissions: configs.connections[idx].permissions }
  } catch (err) {
    return { ok: false, error: '保存失败：' + String(err.message || err) }
  }
}
...
const service = {
  ...
  rerunReport,
  // ★ 新增：指标/维度/数据集
  listMetrics: listMetricsService,
  saveMetric: saveMetricService,
  deleteMetric: deleteMetricService,
  listDimensions: listDimensionsService,
  saveDimension: saveDimensionService,
  deleteDimension: deleteDimensionService,
  listDatasets: listDatasetsService,
  saveDataset: saveDatasetService,
  deleteDataset: deleteDatasetService,
  getMetricSuggestions: getMetricSuggestionsService,
  // ★ 新增：Dashboard
  listDashboards: listDashboardsService,
  getDashboard: getDashboardService,
  saveDashboard: saveDashboardService,
  deleteDashboard: deleteDashboardService,
  duplicateDashboard: duplicateDashboardService,
  addWidget: addWidgetService,
  updateWidget: updateWidgetService,
  removeWidget: removeWidgetService,
  moveWidget: moveWidgetService,
  updateDashboardFilters: updateFiltersService,
  executeDashboardQuery: executeDashboardQueryService,
  getDrillDown: getDrillDownService,
  // ★ 新增：导出
  exportData: exportDataService,
  // ★ 新增：审计
  getAuditLog: getAuditLogService,
  // ★ 新增：权限
  updatePermissions: updatePermissionsService,
}
...
export function apply(ctx) {
  ...
  try {
    ...
    nlQueryToolWithCtx.execute = async (args, exec) => {
      ...
      try {
        ...
        const chart = suggestChartForResult(result.result, question, result.chartType || undefined)
        return {
          ok: true,
          connection: base.connection.name,
          sql: result.sql,
  ...
  // ★ 已扩展：注入 Text2SQL 工具指引 + 指标/维度上下文 + 权限提示。
  try {
      ...
      systemPrompt.context({
        ...
        text: (assemble) => {
          ...
          const lines = [
            ...
            '  · mysql_tables 查看表结构',
            '  · sql_to_chart 对查询结果生成图表规格（支持 bar/line/pie/stat/funnel/scatter/heatmap/radar/sankey/gauge/treemap 等）',
            '  · mysql_execute 写操作（仅 INSERT/UPDATE/DELETE，需连接开启写权限）',
            '- 表名受白名单限制；nl_query 强制只读，绝不会写库。',
          ]
          // 注入行级权限提示
          const perms = connection.permissions
          if (perms && Array.isArray(perms.rowFilters) && perms.rowFilters.length > 0) {
            lines.push('- ⚠️ 数据权限限制：')
            for (const f of perms.rowFilters) {
              if (f.operator === 'IN' && Array.isArray(f.values)) {
                lines.push('  · ' + f.column + ' 只能查看：' + f.values.join(', '))
              } else if (f.value) {
                lines.push('  · ' + f.column + ' ' + f.operator + ' ' + f.value)
              }
            }
            lines.push('  请在生成 SQL 时自动加上对应的 WHERE 条件。')
          }
          if (perms && Array.isArray(perms.columnBlacklist) && perms.columnBlacklist.length > 0) {
            lines.push('- ⚠️ 禁止查看的列：' + perms.columnBlacklist.join('、'))
            lines.push('  请在 SELECT 中排除这些列。')
          }
          // 注入指标/维度上下文（同步读取 cachedMetrics，DSH 框架要求 text 函数必须同步返回 string）
          try {
            const metricsCtx = metrics.buildMetricsContextSync()
            if (metricsCtx) lines.push('\n' + metricsCtx)
          } catch (e) { /* 指标上下文注入失败不影响主功能 */ }
          return lines.join('\n')
        },
      })
