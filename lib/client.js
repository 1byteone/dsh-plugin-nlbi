...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function apply(ctx) {
      ...
      // 4) ★ 右侧栏全能工作台（betterSidebar 服务存在时挂载；不存在则静默跳过）
      //    功能：连接切换 + 智能查询（自然语言即时查+发到对话）+ 数据浏览（表树+网格）+ 报表（生成+收藏+导出）
      if (typeof ctx.inject === 'function') {
        ...
        } catch (err) { /* betterSidebar 不可用则跳过右侧栏，功能仍可从对话与设置页使用 */ }
      }
    }
