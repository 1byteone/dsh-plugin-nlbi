...
/**
 * 生成指标/维度上下文文本（同步版本，供 systemPrompt.context 同步调用）。
 * 在 ensureLoaded() 首次完成之后 cachedMetrics 保证非空，可安全同步读取；
 * DSH 框架的 context().text 只接受同步返回 string，异步函数会返回 Promise
 * 导致下游 text.indexOf 报错（text is not a function）。
 *
 * @returns {string}
 */
export function buildMetricsContextSync() {
  const data = cachedMetrics && Array.isArray(cachedMetrics.metrics) ? cachedMetrics : { metrics: [], dimensions: [], datasets: [] }
  const parts = []
