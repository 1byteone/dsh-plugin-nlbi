...
2. 输出必须是 JSON 格式：{ "sql": "...", "explain": "...", "chartType": "...", "analysisType": "..." }
3. sql 字段：只读 SQL（SELECT/SHOW/DESCRIBE/EXPLAIN），单条语句，不加注释
4. explain 字段：用一句话解释这条 SQL 在查什么，面向非技术用户
5. chartType 字段：推荐的图表类型，可选值：
   - stat: 单一数值统计（如总销售额、用户总数）
   - bar: 分类对比（如各地区销售额）
   - line: 趋势变化（如每日订单量趋势）
   - pie: 占比分布（如各分类占比，类别≤12个时推荐）
   - area: 趋势+规模（如每日访问量面积图）
   - funnel: 漏斗转化（如注册→激活→付费转化）
   - scatter: 相关性分析（如价格vs销量关系）
   - heatmap: 热力分布（如星期×时段访问热力图）
   - radar: 多维能力（如产品各维度评分对比）
   - gauge: 完成度（如目标完成率）
   - stackedBar: 堆叠对比（如各月分类销售堆叠）
6. analysisType 字段：分析类型，可选值：
   - none: 普通查询
   - trend: 趋势分析（时间序列）
   - compare: 对比分析（多维度比较）
   - rank: 排名分析（Top N）
   - proportion: 占比分析（各部分占比）
   - distribution: 分布分析（数据分布特征）
   - funnel: 漏斗分析（转化率）
   - correlation: 相关性分析（两个变量关系）
7. 安全规则：
...
8. 分析能力：
   - 同比分析：用自关联或窗口函数比较去年同期
   - 环比分析：用自关联或窗口函数比较上期
   - 累计分析：用窗口函数 SUM() OVER (ORDER BY ...)
   - 移动平均：用窗口函数 AVG() OVER (ORDER BY ... ROWS BETWEEN N PRECEDING AND CURRENT ROW)
   - Top N：用 ORDER BY ... LIMIT N
   - 占比计算：用子查询或窗口函数 SUM() OVER () 做分母
9. 如果问题不明确，在 explain 中说明你的假设
10. 优先使用已定义的业务指标和维度（如上下文中提供了指标定义）`
...
const FEW_SHOT_EXAMPLES = [
  ...
  {
    question: '各地区销售额占比',
    sql: 'SELECT region, SUM(amount) AS total, ROUND(SUM(amount) / (SELECT SUM(amount) FROM orders) * 100, 2) AS percentage FROM orders GROUP BY region ORDER BY total DESC',
    explain: '计算各地区销售额及其占总销售额的百分比',
    chartType: 'pie',
    analysisType: 'proportion',
  },
  {
    question: '用户注册漏斗：注册→激活→首次购买→复购',
    sql: 'SELECT step, COUNT(DISTINCT user_id) AS users FROM (SELECT user_id, 1 AS step, MIN(created_at) AS t FROM users GROUP BY user_id UNION ALL SELECT user_id, 2 AS step, MIN(activated_at) AS t FROM users WHERE activated_at IS NOT NULL GROUP BY user_id UNION ALL SELECT user_id, 3 AS step, MIN(created_at) AS t FROM orders GROUP BY user_id UNION ALL SELECT user_id, 4 AS step, MIN(created_at) AS t FROM orders WHERE order_no IN (SELECT order_no FROM orders GROUP BY user_id HAVING COUNT(*) > 1) GROUP BY user_id) t GROUP BY step ORDER BY step',
    explain: '分析用户从注册到复购的转化漏斗',
    chartType: 'funnel',
    analysisType: 'funnel',
  },
  {
    question: '价格和销量的关系',
    sql: 'SELECT price, SUM(quantity) AS total_sold FROM products JOIN order_items ON products.id = order_items.product_id GROUP BY price ORDER BY price',
    explain: '分析产品价格与总销量之间的关系',
    chartType: 'scatter',
    analysisType: 'correlation',
  },
  {
    question: '各月销售额同比增长率',
    sql: 'SELECT cur.month, cur.total, prev.total AS prev_total, ROUND((cur.total - prev.total) / prev.total * 100, 2) AS yoy_growth FROM (SELECT DATE_FORMAT(created_at, "%Y-%m") AS month, SUM(amount) AS total FROM orders WHERE YEAR(created_at) = YEAR(CURDATE()) GROUP BY month) cur LEFT JOIN (SELECT DATE_FORMAT(created_at, "%Y-%m") AS month, SUM(amount) AS total FROM orders WHERE YEAR(created_at) = YEAR(CURDATE()) - 1 GROUP BY month) prev ON cur.month = DATE_FORMAT(DATE_CONCAT(prev.month, "-01"), "%Y-%m") ORDER BY cur.month',
    explain: '对比今年和去年各月的销售额，计算同比增长率',
    chartType: 'bar',
...
function normalizeLlmText(raw) {
  ...
  if (Array.isArray(raw)) {
    return raw.map(item => normalizeLlmText(item)).filter(Boolean).join('')
  }
  if (typeof raw === 'object') {
    // 常见 LLM 响应结构：{ content }, { text }, { message: { content } },
    // OpenAI content blocks [{type:'text', text:'...'}]
    if (typeof raw.content === 'string') return raw.content
    if (typeof raw.text === 'string') return raw.text
    if (raw.message && typeof raw.message === 'object') return normalizeLlmText(raw.message.content || raw.message.text)
    if (Array.isArray(raw.content)) return normalizeLlmText(raw.content)
    if (typeof raw.output_text === 'string') return raw.output_text
    if (Array.isArray(raw.output)) return normalizeLlmText(raw.output)
    // 单个 content block
...
/**
 * 解析模型返回的 JSON 文本，兼容纯文本、对象和 content block。
 */
export function parseLlmResult(raw) {
  // DSH 某些模型适配器可能返回 content block 数组、对象或包装对象，
  // 统一转换为字符串后再进行 JSON 提取，避免 text.indexOf is not a function。
  ...
  // 尝试解析 JSON
  // 先找第一个 { 和最后一个 }
  const startIdx = String(text).indexOf('{')
  const endIdx = String(text).lastIndexOf('}')
  ...
  try {
    ...
    const chartType = parsed.chartType || parsed.chart_type || ''
    const analysisType = parsed.analysisType || parsed.analysis_type || ''

    if (!sql || typeof sql !== 'string' || !sql.trim()) {
      return { ok: false, error: '模型未生成 SQL 语句', raw: text }
    }

    return { ok: true, sql: sql.trim(), explain: explain.trim() || '（无说明）', chartType: chartType.trim(), analysisType: analysisType.trim() }
  } catch (err) {
    // JSON 解析失败：尝试更宽松的提取
  ...
  try {
    ...
    if (llm && typeof llm.stream === 'function') {
      ...
      try {
        ...
        const text = normalizeLlmText(blocks.filter((b) => b.type === 'text'))
...
export async function text2sql(ctx, question, connection, pool, options = {}) {
  ...
  try {
    ...
    return {
      ...
      analysisType: llmResult.analysisType || '',
      result: {
        columns,
